...
class ApiTests(APITestCase):
    ...
    def test_create_contact(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {token}')
        data = {'city': 'Msc', 'street': 'Kremlin', 'phone': '+79998887766'}
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_partner_update(self):
        ...
        updated_data = {'partner': user.id,
                        'url': 'https://raw.githubusercontent.com/'
                               'netology-code/python-final-diplom/master/data/shop1.yaml'}
    ...
    def test_post_account_details(self):
        ...
        updated_user = {'last_name': 'new_one'}
        token = Token.objects.create(user=user).key
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {token}')
